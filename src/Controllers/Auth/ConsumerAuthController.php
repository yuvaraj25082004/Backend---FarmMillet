...
class ConsumerAuthController
{
    ...
    public static function register(): void
    {
        ...
        try {
            ...
            $stmt = $db->prepare("
                ...
                VALUES (:email, :mobile, :password, 'consumer', FALSE)
            ");
            ...
            $stmt = $db->prepare("
                INSERT INTO consumer_profiles (user_id, name, street, city, pincode)
                VALUES (:user_id, :name, :street, :city, :pincode)
            ");
            ...
            error_log("Consumer registration failed: " . $e->getMessage());
        ...
        try {
            ...
            $stmt = $db->prepare("
                SELECT u.id, u.email, u.password, u.role, u.is_verified, u.is_active,
                       c.name, c.street, c.city, c.pincode
                FROM users u
            ...
            error_log("Consumer login failed: " . $e->getMessage());
            Response::serverError('Login failed. Please try again'.$e->getMessage());
        }
    }
    ...
    public static function forgotPassword(): void
    {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT id, email FROM users WHERE email = :email AND role = 'consumer'");
            $stmt->execute(['email' => $data['email']]);
            $user = $stmt->fetch();
    ...
    public static function getProfile(): void
    {
        ...
        try {
            ...
            Response::success('Profile fetched', $user);
    ...
    public static function updateProfile(): void
    {
        ...
        try {
            ...
            // Update user table (mobile/email - email update restricted for now)
            if (isset($data['mobile'])) {
                if (!Validator::mobile($data['mobile'])) {
                    Response::validationError(['mobile' => 'Invalid mobile format']);
                }
                $stmt = $db->prepare("UPDATE users SET mobile = :mobile WHERE id = :user_id");
                $stmt->execute(['mobile' => $data['mobile'], 'user_id' => $userData->user_id]);
            }

            // Update consumer_profiles table
            $profileFields = ['name', 'street', 'city', 'pincode'];
            $updateFields = [];
            $params = ['user_id' => $userData->user_id];

            foreach ($profileFields as $field) {
                if (isset($data[$field])) {
                    if ($field === 'pincode' && !Validator::pincode($data[$field])) {
                        Response::validationError(['pincode' => 'Pincode must be 6 digits']);
                    }
            ...
            if (!empty($updateFields)) {
                $sql = "UPDATE consumer_profiles SET " . implode(', ', $updateFields) . " WHERE user_id = :user_id";
                $stmt = $db->prepare($sql);
                $stmt->execute($params);
            }
