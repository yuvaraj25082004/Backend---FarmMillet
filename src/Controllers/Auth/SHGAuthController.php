...
class SHGAuthController
{
    /**
     * Register a new SHG/FPO
     */
    public static function register(): void
    {
        ...
        try {
            ...
            $stmt = $db->prepare("
                ...
                VALUES (:email, :mobile, :password, 'shg', FALSE)
            ");
            ...
            // Insert SHG profile
            $stmt = $db->prepare("
                INSERT INTO shg_profiles (
                    user_id, organization_name, contact_person_name, street, city, pincode,
                    warehouse_location, bank_account_number, bank_ifsc_code
                ) VALUES (
                    :user_id, :organization_name, :contact_person_name, :street, :city, :pincode,
                    :warehouse_location, :bank_account_number, :bank_ifsc_code
                )
            ...
            $stmt->execute([
                'user_id' => $userId,
                'organization_name' => Validator::sanitize($data['organization_name']),
                'contact_person_name' => Validator::sanitize($data['contact_person_name']),
                'street' => Validator::sanitize($data['street']),
                'city' => Validator::sanitize($data['city']),
                'pincode' => $data['pincode'],
                'warehouse_location' => Validator::sanitize($data['warehouse_location'] ?? ''),
            ...
            error_log("SHG registration failed: " . $e->getMessage());
    ...
    public static function login(): void
    {
        ...
        try {
            ...
            $stmt = $db->prepare("
                ...
                WHERE u.email = :email AND u.role = 'shg'
            ");
            ...
            error_log("SHG login failed: " . $e->getMessage());
    ...
    public static function forgotPassword(): void
    {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT id, email FROM users WHERE email = :email AND role = 'shg'");
            $stmt->execute(['email' => $data['email']]);
            $user = $stmt->fetch();
